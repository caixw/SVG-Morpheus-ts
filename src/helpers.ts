			...
			case 'rotate':
				if (transFrom[i] && transTo[i]) {
					...
					for (let j = 0; j < 3; j++) {
						...
						if (Number.isFinite(fromVal) && Number.isFinite(toVal) && Number.isFinite(progress)) {
							res[i]![j] = fromVal + (toVal - fromVal) * progress;
						} else {
							// 如果任何值无效，使用 from 值或默认值
							res[i]![j] = Number.isFinite(fromVal) ? fromVal : 0;
						}
					}
				}
				break;
...
export function trans2string(trans: Transform): string {
	...
	if (trans.rotate && trans.rotate.length === 3) {
		...
		if (Number.isFinite(angle) && Number.isFinite(centerX) && Number.isFinite(centerY)) {
