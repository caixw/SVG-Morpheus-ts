...
export const easings: EasingMap = {
	...
	'elastic-in': (t: number) => {
		...
		const p = 0.3;
		const s = (p / pi2) * asin1;
		return -(2 ** (10 * (t -= 1)) * Math.sin(((t - s) * pi2) / p));
	},
	...
	'elastic-out': (t: number) => {
		...
		const p = 0.3;
		const s = (p / pi2) * asin1;
		return 2 ** (-10 * t) * Math.sin(((t - s) * pi2) / p) + 1;
	},
	...
	'elastic-in-out': (t: number) => {
		...
		const s = (p / pi2) * asin1;
		if (t < 1) {
			return -0.5 * (2 ** (10 * (t -= 1)) * Math.sin(((t - s) * pi2) / p));
		}
		return 2 ** (-10 * (t -= 1)) * Math.sin(((t - s) * pi2) / p) * 0.5 + 1;
	},

	'expo-in': (t: number) => (t === 0 ? 0 : 2 ** (10 * (t - 1))),
	...
	'sine-in-out': (t: number) => 0.5 * (1 - Math.cos(Math.PI * t)),
};
