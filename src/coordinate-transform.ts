 ...
 */
export function calculateTransformMatrix(fromViewBox: ViewBoxInfo, toViewBox: ViewBoxInfo): TransformMatrix {
...
export function transformPoint(x: number, y: number, matrix: TransformMatrix): [number, number] {
	return [x * matrix.scaleX + matrix.translateX, y * matrix.scaleY + matrix.translateY];
}
...
export function calculateOptimalViewBox(
	fromViewBox: ViewBoxInfo | undefined,
	toViewBox: ViewBoxInfo | undefined,
): ViewBoxInfo {
	// 如果都没有 ViewBox，使用默认
...
function transformGradientCoordinates(gradientStr: string, matrix: TransformMatrix): string {
	...
	if (gradientStr.includes('linearGradient')) {
		...
		transformed = transformed.replace(/\b(x1|y1|x2|y2)=["']([^"']+)["']/g, (_match, attr, value) => {
	...
	if (gradientStr.includes('radialGradient')) {
		// 匹配 cx, cy, r, fx, fy 属性
		transformed = transformed.replace(/\b(cx|cy|fx|fy|r)=["']([^"']+)["']/g, (_match, attr, value) => {
			const transformedValue = transformGradientValue(attr, value, matrix);
			return `${attr}="${transformedValue}"`;
		});
	}
...
function transformPatternCoordinates(patternStr: string, matrix: TransformMatrix): string {
	...
	// 处理 pattern 的坐标属性
	transformed = transformed.replace(/\b(x|y|width|height)=["']([^"']+)["']/g, (_match, attr, value) => {
		const transformedValue = transformGradientValue(attr, value, matrix);
		return `${attr}="${transformedValue}"`;
	});
