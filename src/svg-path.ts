...
export function parsePathString(pathString: any): any {
	...
	if (Array.isArray(pathString)) {
		...
		String(pathString).replace(pathCommand, (_, b, c) => {
			...
			c.replace(pathValues, (_: string, b: string) => {
				if (b) params.push(+b);
				return '';
			});
...
export function path2curve(path: any, path2?: any): any {
	...
	const fixM = (path1: any, path2: any, a1: any, a2: any, i: any) => {
...
export function curvePathBBox(path: any): any {
	...
	for (let i = 0, ii = path.length; i < ii; i++) {
		...
		if (p[0] == 'M') {
			...
			const x0 = x,
				y0 = y;
			const x1 = Number.isFinite(p[1]) ? p[1] : 0;
			const y1 = Number.isFinite(p[2]) ? p[2] : 0;
			const x2 = Number.isFinite(p[3]) ? p[3] : 0;
			const y2 = Number.isFinite(p[4]) ? p[4] : 0;
			const x3 = Number.isFinite(p[5]) ? p[5] : 0;
			const y3 = Number.isFinite(p[6]) ? p[6] : 0;

			const dim = curveDim(x0, y0, x1, y1, x2, y2, x3, y3);
			if (
				Number.isFinite(dim.min.x) &&
				Number.isFinite(dim.max.x) &&
				Number.isFinite(dim.min.y) &&
				Number.isFinite(dim.max.y)
			) {
